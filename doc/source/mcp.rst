...
Das fuer den Durchlauf relevante Verzeichnis bekommen wir mit

.. code-block:: bash

	python3 -c "import importlib.resources as r; print(r.files('sdv.doc.waterloo'))"

Diesen Pfad nennen wir im folgenden :wtrl_file:`${ROOT}`.

Im Verzeichnnis :wtrl_file:`${ROOT}/etc` befinden sich unter anderem ein Konfigurationsfile
...
Diese Konfiguration konfiguriert einen MCP-Server mit dem Transport :wtrl_value:`streaming-http`,
erreichbar ueber den Port 13316. Diese Portnummer wird in das Dockerimage uebernommen
und 'exposed'.

Bake-Modus (:wtrl_opt:`--bake-roots`, implizit)
...............................................

Wir verwenden diese Konfiguration als Beispiel, um den MCP-Server im Dockercontainer zu betreiben:

.. code-block:: bash

	waterlint render-docker --in ${ROOT}/etc/wtrl_mcp.http.toml --out /tmp/my_wtrl_mcp

Hierbei werden das Dockerfile :wtrl_file:`/tmp/my_wtrl_mcp` und ein Build-Skript :wtrl_file:`/tmp/build.my_wtrl_mcp.sh` erzeugt.
Das Dockerimage erzeugt man nun einfach durch den Aufruf des Build-Skripts. Per default werden die
Roots des MCP-Servers in das Image gebacken.

.. code-block:: bash

	/tmp/build.my_wtrl_mcp.sh

Danach sollte Docker das Image kennen:
...
und wir koennen es testweise im Vordergrund starten. Hierbei ist wichtig,
dass wir die konfigurierte Portnummer (in unserem Beispiel 13316) weiterleiten.

.. code-block:: bash

	docker run --rm -p 13316:13316 wtrl-mcp-my_wtrl_mcp

Der MCP-Server wird sich dann etwa so melden (umgebrochen fuer bessere Lesbarkeit):
...
Alternativ koennen wir den Container als Daemon betreiben:

.. code-block:: bash

	docker run -d --name wtrl-mcp-my_wtrl_mcp -p 13316:13316 wtrl-mcp-my_wtrl_mcp

und wie in Docker ueblich mit
...
No-Bake-Modus (:wtrl_opt:`--no-bake-roots`)
...........................................

Wenn die Roots des MCP-Servers nicht im Image landen sollen, erzeugt man das Dockerfile so:

.. code-block:: bash

	waterlint render-docker --in ${ROOT}/etc/wtrl_mcp.http.toml --out /tmp/my_wtrl_mcp --no-bake-roots

Im Gegensatz zum Bake-Modus wird hierbei zusaetzlich ein Launch-Skript :wtrl_file:`/tmp/launch.my_wtrl_mcp.sh`
erzeugt, das man aufruft, um den Container zu starten. Dieses Skript ist erforderlich, weil
durch die externen Roots des MCP-Servers der Start des Containers von Hand unpraktikabel waere.
Mit
...
wird das Image gebaut und der Container gestartet. Das Logging erfolgt nach :wtrl_lit:`stdout`.
Der Container laeuft im Vordergrund und wird einfach mit :wtrl_key:`CTRL C` gestoppt.
